...
class SoftmaxPrecisionTester:
    """Test softmax precision with fp32 vs fp64 ground truth"""
    ...
    def softmax_fp64_cpu(self, x: torch.Tensor) -> torch.Tensor:
        """Ground truth: softmax with fp64 on CPU"""
        x_fp64 = x.to(dtype=torch.float64, device='cpu')
        return torch.nn.functional.softmax(x_fp64, dim=-1)
    
    def softmax_fp32_cpu(self, x: torch.Tensor) -> torch.Tensor:
        """Test 1: softmax with fp32 on CPU"""
        x_fp32 = x.to(dtype=torch.float32, device='cpu')
        return torch.nn.functional.softmax(x_fp32, dim=-1)
    
    def softmax_fp32_gpu(self, x: torch.Tensor) -> torch.Tensor:
        """Test 2: softmax with fp32 on GPU (cuDNN)"""
        if not torch.cuda.is_available():
            return None
        x_fp32 = x.to(dtype=torch.float32, device='cuda')
        result = torch.nn.functional.softmax(x_fp32, dim=-1)
        return result
    
    def generate_test_cases(self) -> List[Tuple[str, torch.Tensor]]:
        ...
        test_cases.append((
            ...
            torch.randn(4, 128) * 50
        ))
        
        # Case 5: Very large positive values (fp32 overflow)
        test_cases.append((
            "Very large positive [80, 120]",
            torch.rand(4, 128) * 40 + 80
        ))
        
        # Case 6: Near fp32 max (3.4e38)
        test_cases.append((
            "Near fp32 max",
            torch.tensor([[1e30, 1e31, 1e32, 1e33]] * 32)
        ))
        
        # Case 7: One very large value with small values
        case7 = torch.ones(4, 128) * (-10)
        case7[:, 0] = 50  # One dominant value
        test_cases.append((
            "One dominant large value",
            case7
        ))
        ...
        test_cases.append((
            "Large sequence [2048]",
            torch.randn(2, 2048) * 10
        ))
        
        # Case 11: Alternating large/small values
        case11 = torch.zeros(4, 128)
        case11[:, ::2] = 50  # Even positions
        case11[:, 1::2] = -50  # Odd positions
        test_cases.append((
        ...
        test_cases.append((
            ...
            torch.tensor([[16777216.0, 16777217.0, 16777218.0, 16777220.0]] * 32)
        ))
        
        # Case 16: Gradient-like values (small)
        test_cases.append((
            "Gradient-like values [1e-4, 1e-2]",
            torch.rand(4, 128) * 0.01 + 1e-4
        ))
    ...
    def run_all_tests(self):
        """Run all test cases"""
        print("\n" + "="*80)
        print("SOFTMAX FP32 PRECISION TEST")
        print("Ground Truth: softmax-fp64-cpu (PyTorch)")
        print("Under Test: 1) softmax-fp32-cpu (PyTorch)")
        print("            2) softmax-fp32-gpu (cuDNN)")
        print("="*80)
        ...
        for case_name, x in test_cases:
            try:
                self.run_single_test(case_name, x)
