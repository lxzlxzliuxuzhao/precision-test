...
class RMSNormPrecisionTester:
    """Test RMSNorm precision with fp32 vs fp64 ground truth"""
    ...
    def rmsnorm_fp64_cpu(self, x: torch.Tensor, weight: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
        """Ground truth: RMSNorm with fp64 on CPU"""
        x_fp64 = x.to(dtype=torch.float64, device='cpu')
        weight_fp64 = weight.to(dtype=torch.float64, device='cpu')
        
        # Create RMSNorm module in fp64
        rmsnorm = torch.nn.RMSNorm(x.shape[-1], eps=eps, dtype=torch.float64, device='cpu')
        rmsnorm.weight.data = weight_fp64
        
        return rmsnorm(x_fp64)
    
    def rmsnorm_fp32_cpu(self, x: torch.Tensor, weight: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
        """Test 1: RMSNorm with fp32 on CPU"""
        x_fp32 = x.to(dtype=torch.float32, device='cpu')
        weight_fp32 = weight.to(dtype=torch.float32, device='cpu')
        
        # Create RMSNorm module in fp32
        rmsnorm = torch.nn.RMSNorm(x.shape[-1], eps=eps, dtype=torch.float32, device='cpu')
        rmsnorm.weight.data = weight_fp32
        
        return rmsnorm(x_fp32)
    
    def rmsnorm_fp32_gpu(self, x: torch.Tensor, weight: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
        """Test 2: RMSNorm with fp32 on GPU"""
        if not torch.cuda.is_available():
            return None
        x_fp32 = x.to(dtype=torch.float32, device='cuda')
        weight_fp32 = weight.to(dtype=torch.float32, device='cuda')
        
        # Create RMSNorm module in fp32 on GPU
        rmsnorm = torch.nn.RMSNorm(x.shape[-1], eps=eps, dtype=torch.float32, device='cuda')
        rmsnorm.weight.data = weight_fp32
        
        return rmsnorm(x_fp32)
    
    def generate_test_cases(self) -> List[Tuple[str, torch.Tensor, torch.Tensor]]:
        """Generate comprehensive test cases for LLM training scenarios
        Returns: List of (case_name, input, weight)
        """
        test_cases = []
        ...
        weight = create_weight(hidden_size)
        test_cases.append((
        ...
        # Case 5: Very small RMS (numerical stability challenge)
        weight = create_weight(hidden_size)
        test_cases.append((
            "Very small RMS (1e-7)",
            torch.randn(4, 128, hidden_size) * 1e-7,
            weight
        ))
        ...
        # Case 7: All equal values (constant RMS)
        weight = create_weight(hidden_size)
        test_cases.append((
            "All equal values",
        ...
        large_hidden = 4096
        weight = create_weight(large_hidden)
        test_cases.append((
        ...
        # Case 17: Alternating high/low magnitude across features
        weight = create_weight(hidden_size)
        case17 = torch.randn(4, 128, hidden_size)
        case17[:, :, :hidden_size//2] *= 0.01  # Low magnitude
        case17[:, :, hidden_size//2:] *= 10.0  # High magnitude
        test_cases.append((
            "Mixed feature magnitude",
            case17,
            weight
        ))
        ...
        # Case 21: All zeros (edge case)
        weight = create_weight(hidden_size)
        test_cases.append((
            "All zeros (edge case)",
            torch.zeros(4, 128, hidden_size),
            weight
        ))
        
        # Case 22: One feature dimension very different
        weight = create_weight(hidden_size)
        case22 = torch.randn(4, 128, hidden_size) * 1e-3
        case22[:, :, 0] = 100.0  # One dimension dominates
        test_cases.append((
            "One dominant feature dimension",
            case22,
            weight
        ))
        
        # Case 23: High precision requirement (small differences)
        weight = create_weight(hidden_size)
        base = torch.ones(4, 128, hidden_size) * 10.0
        noise = torch.randn(4, 128, hidden_size) * 1e-6
        test_cases.append((
            "High precision (1e-6 noise)",
            base + noise,
            weight
        ))
        
        # Case 24: Alternating signs
        weight = create_weight(hidden_size)
        case24 = torch.randn(4, 128, hidden_size).abs()
        case24[:, :, ::2] *= -1  # Negative for even indices
        test_cases.append((
            "Alternating signs",
            case24 * 5,
            weight
        ...
        # Case 25: Near eps values (epsilon edge case)
        weight = create_weight(hidden_size)
        test_cases.append((
            "Near eps values (1e-7)",
            torch.randn(4, 128, hidden_size) * 1e-7,
            weight
        ))
    ...
    def run_all_tests(self):
        """Run all test cases"""
        print("\n" + "="*80)
        print("RMSNORM FP32 PRECISION TEST")
        print("Ground Truth: rmsnorm-fp64-cpu (PyTorch)")
        print("Under Test: 1) rmsnorm-fp32-cpu (PyTorch)")
        print("            2) rmsnorm-fp32-gpu")
        print("="*80)
        
        test_cases = self.generate_test_cases()
        
        for case_name, x, weight in test_cases:
            try:
                self.run_single_test(case_name, x, weight)
